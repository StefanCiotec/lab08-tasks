...
static struct kmem_cache * minfs_inode_cachep;
...
static struct inode *minfs_alloc_inode(struct super_block *s)
{
	...
	mii = (struct minfs_inode_info *)kzalloc(sizeof(*mii), GFP_KERNEL);
	if (!mii)
		return NULL;
...
static void minfs_destroy_inode(struct inode *inode)
{
	...
	struct minfs_inode_info *mmi;

	mmi = container_of(inode, struct minfs_inode_info, vfs_inode);
